...
class MyDLmodel:
    def __init__(self, model, device, weight_decay=0.01, dropout_prob=0.1, num_training_steps=None):
        self.model = model
        self.dropout = nn.Dropout(p=dropout_prob)  # 显式定义 Dropout 层
        self.device = device
        self.model.to(device)

        # 优化器和学习率调度器
        self.optimizer = torch.optim.AdamW(self.model.parameters(), lr=4e-5, weight_decay=weight_decay)
        ...
        class_weights[0] *= 1.4  # 调整权重
        class_weights[2] *= 1.4
        weights_tensor = torch.tensor(class_weights, dtype=torch.float32).to(device)
        self.criterion = FocalLoss(alpha=1, gamma=2, weights=weights_tensor)

    def train(self, dataloader_train, dataloader_dev, epochs, adv_epsilon=1e-5, adv_alpha=1e-2):
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(dataloader_train):
                ...
                logits = self.dropout(outputs.logits)  # 在 logits 上应用 Dropout
                ...
                # Step 2: 对抗训练
                self._apply_adversarial_training(input_ids, attention_mask, labels, adv_epsilon, adv_alpha)
    ...
    def _apply_adversarial_training(self, input_ids, attention_mask, labels, adv_epsilon, adv_alpha):
        # 获取嵌入层的梯度
        self.model.zero_grad()
        outputs = self.model(input_ids, attention_mask=attention_mask)
        ...
        for param in self.model.bert.embeddings.parameters():
            if param.grad is not None:
                ...
                perturbation = adv_epsilon * grad / (torch.norm(grad, dim=-1, keepdim=True) + 1e-8)
                param.data.add_(adv_alpha * perturbation)  # 添加扰动
...
if __name__  == "__main__":
    ...
    writer = SummaryWriter('/old_home/lyt/zxj_workplaces/ailab/CodeWithDataset/lab3/log/pert')
    # 计算总训练步数
    epochs = 20
