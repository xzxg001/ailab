    ...
    def train(self, dataloader_train, dataloader_dev, epochs):
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(dataloader_train):
                ...
                # 提取每一层的CLS向量
                cls_vectors = torch.stack([hidden_states[i][:, 0] for i in range(1, 13)], dim=1)  # (N, 12, 768)

                # 通过TextCNN进行分类
                logits = self.textcnn(cls_vectors)
    ...
    def evaluate(self, dataloader_val):
        ...
        for batch in dataloader_val:
            ...
            with torch.no_grad():
                ...
                cls_vectors = torch.stack([hidden_states[i][:, 0] for i in range(1, 13)], dim=1)
                logits = self.textcnn(cls_vectors)
                loss = self.criterion(logits, labels)
...
if __name__  == "__main__":
    ...
    writer = SummaryWriter('/old_home/lyt/zxj_workplaces/ailab/CodeWithDataset/lab3/log/clscnn')
    # 计算总训练步数
    epochs = 20
