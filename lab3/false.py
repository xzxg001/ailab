    ...
    def train(self, dataloader_train, dataloader_dev, epochs):
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(dataloader_train):
                ...
                bert_features = outputs.hidden_states[-1]  # Use the last hidden state from BERT

                # Pass BERT features through TextCNN
...
if __name__  == "__main__":
    ...
    test_dataset = TensorDataset(test_coded_txt["input_ids"], test_coded_txt["attention_mask"])
    test_dataloader = DataLoader(test_dataset, batch_size=32)

# Predict using the initial model
    test_preds = mymodel.predict(test_dataloader)
# Combine original train texts and labels with test texts and pseudo-labels
    combined_txt = train_txt + test_txt
    combined_labels = train_label + list(test_preds)

# Re-tokenize and create new combined dataset
    combined_coded_txt = text_tokenize(combined_txt)
    combined_dataset = TensorDataset(combined_coded_txt["input_ids"], 
                                 combined_coded_txt["attention_mask"],
...
# Recompute total_steps with new dataloader
    total_steps = len(combined_dataloader_train) * epochs
    mymodel = MyDLmodel(pretrained_model, device, num_training_steps=total_steps)

# Retrain the model
    mymodel.train(combined_dataloader_train, dataloader_dev, epochs)
