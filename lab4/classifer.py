...
#         self.attention = MultiheadAttention(embed_dim=input_dim, num_heads=num_heads, batch_first=True)
...
#         x = self.layer_norm1(attn_output + x)  # Skip connection + LayerNorm

#         # 2. Feed Forward Network
#         ff_output = F.relu(self.fc1(x))
#         ff_output = self.fc2(ff_output)
#         x = self.layer_norm2(ff_output + x)  # Skip connection + LayerNorm

#         return x

# class CombinedModel(nn.Module):
#     def __init__(self, pretrained_model, num_labels=4, num_heads=8, hidden_dim=1024, num_layers=3):
#         super(CombinedModel, self).__init__()
#         self.pretrained_model = pretrained_model
...
#         # 处理文本和音频特征融合的 Transformer 层
#         self.transformer = TransformerBlock(input_dim=2 * hidden_dim, hidden_dim=hidden_dim * 2, num_heads=num_heads, num_layers=num_layers)

#         # 修改分类器的输入维度为 2 * hidden_dim
#         self.classifier = nn.Linear(2 * hidden_dim, num_labels)  # 将这里的 hidden_dim 改为 2 * hidden_dim
...
#         # 将文本特征投影到更高维度
#         text_features = self.text_projection(text_features)  # Shape: (batch_size, hidden_dim)

#         # 拼接文本和音频特征
#         combined_features = torch.cat((text_features, audio_features), dim=1)  # Shape: (batch_size, 2*hidden_dim)

#         # 通过 Transformer 层处理
#         combined_features = combined_features.unsqueeze(1)  # Shape: (batch_size, 1, 2*hidden_dim)
#         transformer_output = self.transformer(combined_features)  # Shape: (batch_size, 1, hidden_dim)

#         # 展平特征以供分类
#         transformer_output = transformer_output.squeeze(1)  # Shape: (batch_size, hidden_dim)

#         # 分类器现在期望的输入维度是 2 * hidden_dim
#         logits = self.classifier(transformer_output)  # Shape: (batch_size, num_labels)
...
if __name__ == "__main__":
    ...
    mymodel = MyDLmodel( model, device, 
        ...
        lr=1.8991294150802777e-05, 
        alpha=1.9804152891237692, 
        gamma=1.4789610739401222,
        weight_decay=0.020968676503784745
    )
    #Best Hyperparameters: {'alpha': , 'gamma': , 'lr': , 'weight_decay': }
    writer = SummaryWriter('/old_home/lyt/zxj_workplaces/ailab/CodeWithDataset/lab4/log/final')
